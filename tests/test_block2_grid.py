"""Does the block 2 v2 grid do what PREREGISTRATION.md section 4 says?

Ported from ``block2_development/tests/test_grid_behaviour.py`` onto
the vendored ``aster_pipeline.block2`` copy. Synthetic sessions with known compositions,
run through the real ``evaluate()``. These are behaviour tests, not unit tests: each case
is a clinical situation and the expected verdict is the one the pre-registration commits
to. If a case fails, either the code or the frozen grid document is wrong -- never adjust
...
from aster_pipeline.block2.grid import QUANTITIES, Thresholds, evaluate
from aster_pipeline.block2.quantify import ClassRates, Quantifier
...
def test_uncorrected_counts_would_fire_on_a_normal_smear():
    ...
    """
    poor = Quantifier(rates={name: ClassRates(0.90, 0.06, 2000, 20000) for name in QUANTITIES})
...
def test_a_threshold_near_the_classifier_fpr_is_not_assertable():
    ...
    ``baso_frac >= 0.02`` sits barely above a 3 % false-positive rate. A session showing
    5 % basophils - which would be real basophilia - cannot assert the criterion, because
    the correction attributes most of the observation to classifier error. The grid
    returns indeterminate, which is the honest answer.
    """
