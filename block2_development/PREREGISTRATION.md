...
### 8a. The blast-lineage band is fitted on pseudo-bags, as §5.2 declared

§5.2 declares `θ_lo`, `θ_hi` "set on cell-level pseudo-bags at equal error rate". The first
implementation fitted them on **single cells** instead, with an undeclared ±0.15 around the
cell-level EER. It produced `[0.01, 0.31]`.

That is the wrong level. At inference `lineage_post` is a **session aggregate** — the summed
lymphoblast probability over the summed lymphoblast + myeloblast probability, taken over the
cells predicted as blasts. A typical myeloblast keeps ~5 % lymphoid probability, so an AML
session sits near 0.05: inside the cell-level band, i.e. `lineage_indeterminate`, for almost
every AML patient.

Corrected: 1 000 pseudo-bags per lineage, drawn from the validation blasts and aggregated
exactly as at inference. The bag size is **derived, not chosen**: a session reaches the
lineage decision only at the pattern tier, `N_c ≥ 200` **[STD]**, and only once blasts reach
20 % **[WHO]** — so the smallest blast population that ever gets a lineage call is
**200 × 0.20 = 40 blasts**. The notebook computes it from the frozen values. Each bound caps one error at **5 %**, the
convention already used for `τ_abn` and `θ_APL`:

- `θ_lo` = 5th percentile of lymphoid bags — at most 5 % of ALL bags called myeloid;
- `θ_hi` = 95th percentile of myeloid bags — at most 5 % of AML bags called lymphoid;
- if `θ_lo ≥ θ_hi` the lineages are separable and a single cut replaces the band.

The share of AML and ALL pseudo-bags falling in each label is reported
(`results/lineage_band.json`). The smallest bag is the noisiest, so a band fitted at 40 is
conservative for every larger session.

The pseudo-bags pool cells from several patients, as §5.2 already declared: the band is
fitted on a construct, not on patients.

### 8b. Three other scale mismatches in §7, caught before they touched a result

| Value | Fault | Fix |
|---|---|---|
| `τ_abn`, `θ_APL` | `cuts[np.argmax(fpr <= 0.05)]` returns roc_curve's first threshold, `+inf` | last threshold with FPR ≤ 0.05 |
| `τ_abn` | fitted on raw MIL probabilities, compared with the calibrated `P_abn` | mapped through the temperature scaler |
| `[REF]` intervals | computed on raw argmax fractions over 16 classes, compared with quantifier-corrected fractions over `N_c` | computed on corrected fractions over `N_c` |

None of them had reached a decision: they were found while §7 was being run cell by cell.
All four faults share one cause — a value fitted at one scale or level and compared at
another — and each fix puts the fitted value on the scale of the quantity it is compared
with.
...
**Status: secondary analysis, declared and hashed before it is run, run once.** The primary
result of §6.3 is the frozen grid as it ran. It is not replaced, it is reported first, and
every table of this amendment is reported next to it, never instead of it.

### 9a. What was observed, and what it was caused by

On cAItomorph, R4 (`non_leukemic`) was blocked on most stem-cell donors by its blast
criterion: their predicted blast fraction had a median of 11.2 % (p90 21.5 %), so
`blast_frac < 5 %` could not be asserted. The same offset was **visible before the test**, on
the AML-MLL controls: predicted blast fraction median 11.9 %, p90 17.8 %, max 30.4 %, for 60
patients whose manual 100-cell differential counts **zero** blasts. It was noticed and not
acted on; this amendment acts on it.

Cause. The quantifier of 6a inverts TPR and FPR measured on **single validation cells** of
the cell-level sources. Those rates do not transfer to **whole sessions** of another
acquisition domain: on AML-MLL crops the cell head calls ~12 % of normal leukocytes blasts,
which no cell-level rate predicts. The error is a session-level, domain-level offset, so it
is corrected at that level.

### 9b. The model — ACC one level up

Per patient, with `q` the observed blast fraction and `m` the manual one,

    q = a + b · m + patient-level scatter          corrected = (q − a) / b

`a` is the session-level false-positive rate, `b` the session-level TPR − FPR: the model of
6a, fitted across patients instead of across cells (`src/aster_block2/session_calibration.py`).

| Element | Declared choice |
|---|---|
| Population | **all 189 AML-MLL patients** (60 control, 129 AML). The cell head has never seen AML-MLL (no cell labels), and the thresholds `blast_frac` meets are **[WHO]** 0.20 and **[CONV]** 0.05 — nothing fitted on `dev_fit`/`dev_cal` depends on it. cAItomorph takes no part. |
| `q` | argmax counts of `myeloblast + lymphoblast + promyelocyte_abnormal` over `N_c`, on **all** crops of the patient — exactly what `SessionScorer` counts |
| `m` | WHO blast equivalents of the manual count: `pb_myeloblast / pb_total`; for PML::RARA patients `(pb_myeloblast + pb_promyelocyte) / pb_total`, since the promyelocytes counted in APL blood are the abnormal ones |
| Fit | ordinary least squares, one row per patient; covariance of `(a, b)` from 2 000 patient-level bootstrap resamples |
| Scatter | quasi-binomial dispersion `φ` = the **larger** of the Pearson dispersions of the control and AML strata (≥ 1), so a stratum that scatters more is never averaged away |
| Reference error | the manual count is itself a 100-cell sample; the regression-dilution ratio is reported, not corrected |
| Propagation | `Var = [φ·q̃(1−q̃)/n + Var a + p̂²·Var b + 2p̂·Cov(a,b)] / b²`, with `q̃ = (k+½)/(n+1)`, turned into an effective `(k, n)` by the design effect of 6a, capped at `n` |
| Scope | **`blast_frac` only** (R1 and R4). `myeloblast_frac`, `abn_promy_frac` (R1a) and every other quantity keep the cell-level quantifier |

### 9c. Conditions declared before the run

1. **Applicability.** The recalibration is used only if the lower bound of the 95 %
   bootstrap interval of `b` is ≥ 0.05, the separation floor of 6a. Otherwise it is
   reported as *not applicable* and nothing else is run.
2. **Replay.** Re-running the frozen grid from the stored per-patient inputs with the
   original quantifier must reproduce all 409 primary labels exactly, before the
   recalibrated one is run. Any mismatch stops the analysis.
3. **Internal check on AML-MLL, before cAItomorph.** 5-fold patient-level cross-validation,
   stratified by diagnosis: share of held-out controls on which `blast_frac ≥ 5 %` is
   REJECTED, and share of held-out AML with manual blasts ≥ 20 % on which `blast_frac ≥ 20 %`
   is ASSERTED — with the cell-level quantifier and with the recalibration.
4. **cAItomorph, once.** Reported: the full diagnosis × label table; the `non_leukemic` share
   among donors and among reactive patients; specificity against donors (no leukaemic
   label); sensitivity for acute leukaemia (AML, ALL, AL → an `acute_blastic` label); the
   abstention rate — each with its Wilson 95 % interval, frozen and amended side by side.
   Every patient whose label changes is listed; any donor or reactive patient that receives
   a leukaemic label it did not have is named.
5. **No success bar, no second variant.** Nothing is refitted after cAItomorph has been seen,
   and whatever the result, it is reported.

**Assumption, declared not verified.** The offset measured on AML-MLL is taken to be the
offset of cAItomorph. The two controls populations sit at similar medians (11.9 % and
11.2 %), but checking it would need cAItomorph manual differentials, and none is used.

### 9d. What this does not change

γ, the reference percentile, the tier sizes, every rule, every **[WHO]**, **[STD]** and
**[CONV]** value, and every resolved **[REF]** and **[FIT]** value (`τ_abn`, the lineage
cut, the OOD threshold, `N_c/N`, the reference intervals). The MIL head, `P_abn` and its
AUROC are untouched: `P_abn` does not use `blast_frac`.
